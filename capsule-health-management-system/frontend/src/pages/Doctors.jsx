...
const Doctors = () => {
  ...
  // ---- FIX for DOM XSS in doctor image ----
  const getSafeImageUrl = (url) => {
    // allow only http/https URLs, otherwise use a trusted placeholder
    return url && /^https?:\/\/[^\s]+$/i.test(url) ? url : assets.doctor
  }
        ...
        <div className='w-full grid grid-cols-auto gap-4 gap-y-6'>
          {filterDoc.map((item, index) => (
            <div
              ...
              key={index}
            >
              {/* ✅ FIXED: Sanitize the doctor image URL */}
              <img
                className='bg-[#EAEFFF]'
